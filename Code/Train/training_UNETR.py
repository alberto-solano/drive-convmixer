...
if not os.path.exists(os.path.join("..", "Checkpoints_UNETR", TRAINING_FOLDER)):
    os.makedirs(os.path.join("..", "Checkpoints_UNETR", TRAINING_FOLDER))
    os.makedirs(os.path.join("..", "Checkpoints_UNETR", TRAINING_FOLDER, "image_predictions"))
...
if LOAD_CHECKPOINT:
    ...
    training_logs = torch.load("../Checkpoints_UNETR/{}/training_logs.pt".format(TRAINING_FOLDER))
...
for epoch in range(NUM_EPOCHS):
    ...
    if check:
        ...
        save_checkpoint(checkpoint, "../Checkpoints_UNETR/{}/my_check.pth.tar".format(TRAINING_FOLDER), best_metric = training_logs["best_"+CONTROL_METRIC])
        save_predictions_as_imgs(val_loader, model,
                                 folder="../Checkpoints_UNETR/{}/image_predictions".format(TRAINING_FOLDER),
                                 device=DEVICE)

    training_logs["saved"].append(check)

    if not epoch % 20 or epoch == (NUM_EPOCHS - 1):
        torch.save(training_logs, "../Checkpoints_UNETR/{}/training_logs.pt".format(TRAINING_FOLDER))
...
torch.save(params, "../Checkpoints_UNETR/{}/training_params.pt".format(TRAINING_FOLDER))
